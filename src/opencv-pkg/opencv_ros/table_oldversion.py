...
class Table(Node):
    ...
    def __init__(self):
        ...
        self.img_sub   = self.create_subscription(Image, color_image_topic, self.image_callback, qos, callback_group=cb_group2)
        ...
        # ---------- Service ----------
        self.server = self.create_service(KeyVisual, 'table', self.table_callback, callback_group=cb_group1)
    ...
    def image_callback(self, msg: Image):
        try:
            ...
            now = time.time()
            if not self.start and now - self.last_log_time > 30.0:
                ...
                while time.time() < deadline:
                    ...
                    if self.ok and self.location is not None and ready_intrinsics:
                        ...
                        if Z is not None and (self.min_valid_z <= Z <= self.max_valid_z):
                            ...
                            self.get_logger().info(f"Table detected: result OK, Z={Z:.3f}m, move (dx,dy)=({X:.3f},{-Y:.3f})m")
                            self.start = False
                            return response
                    
                    time.sleep(0.05)  # 讓出執行時間給其他 callback
                
                self.get_logger().warn('Table detection timeout - no valid result found')
                response.ok = False
                self.start = False
                return response
            else:
